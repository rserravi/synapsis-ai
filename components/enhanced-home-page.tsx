  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Hero Section - Solo cuando no hay fichas y no hay filtros */}
        {pagination.total === 0 && !loading && !hasFilters && (
          <motion.div
        ...
        {/* Stats and Content - Cuando hay fichas */}
        {(pagination.total > 0 || loading || hasFilters) && (
          <>
            {/* Stats */}
            {pagination.total > 0 && (
            ...
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center space-x-4">
                <h2 className="text-2xl font-bold text-slate-900 dark:text-slate-100">
                  {hasFilters ? 'Resultados de Búsqueda' : 'Mis Fichas'}
                </h2>
                <TagManager
                  trigger={
            ...
            {/* Results */}
            {loading ? (
              <LoadingSpinner message="Buscando fichas..." />
            ) : cards.length > 0 ? (
              <CardGrid cards={cards} />
            ) : (
              <EmptyState
                searchQuery={filters.query}
                selectedTag={filters.tags[0] || null}
